...
public class ScoringService {
    ...
    public Map<String, Object> findDirectReportScoring(DebiturEvent debiturEvent, SseEmitter emitter) {
        ...
        try {
            ...
            event.setNextTopic("scoring-pub");

            String token = getTokenUpdate();
            if (null != token) {
    ...
    public void getDataScoring(ScoringEvent event) {
        log.debug("[{}] Get Token ", event.getEventId());
        String token = getTokenUpdate(); // getToken(username, password, event.getEventId());
        TempReport tempReport = reportRepository.findByIdReport(event.getTempReportId()).orElse(null);
        if(null != token){
            log.debug("[{}] Token : {}", event.getEventId(), token);
            String response = triggerScoringUpdate(event, token, tempReport);
            log.debug("[{}] Response : {}", event.getEventId(), response);
        }else{
            log.info("[{}] Token is empty", event.getEventId());
            if (null != tempReport){
                saveAndSendErrorNotif(event, tempReport, HttpStatus.INTERNAL_SERVER_ERROR.value(),
                        "Failed Scoring : Response Token Scoring Engine was Empty");
            }
        }
    }

    /** After deploy new domain */
    ...
    public String triggerScoringUpdate(ScoringEvent event, String token, TempReport tempReport){
        ...
        if (null != tempReport){
            try {
                return requestForScoringEngine(event, tempReport, bearerAuth);
    ...
    private String requestForScoringEngine(ScoringEvent event, TempReport tempReport, String bearerAuth) {
...
//    private void responseSuccesScoring(ScoringEvent event, TempReport tempReport) throws JsonProcessingException {
//        if (null != tempReport.getDataScore()) {
...
//    }
    ...
    private String getResultCommonReport(ScoringEvent event, String scoringUrl, String bearerAuth,
        ...
        String finalScoringUrl = scoringUrl;
